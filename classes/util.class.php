...
//    $cadena = str_replace($remove, ' ', $cadena);
	...
	function setFormatDate($fecha, $full = false){
	...
	function EnumEstados(){
	
		$sql = 'SELECT * FROM estado ORDER BY nombre ASC';
		$this->DB()->setQuery($sql);		
		$estados = $this->DB()->GetResult();
				
		return $estados;
	}
	
	function GetNomEstado($estadoId){
	
		$sql = 'SELECT nombre FROM estado WHERE estadoId = "'.$estadoId.'"';
		$this->DB()->setQuery($sql);		
		$estado = $this->DB()->GetSingle();
	...
	function GetNomMunicipio($municipioId){
	
		$sql = 'SELECT nombre FROM municipio WHERE municipioId = "'.$municipioId.'"';
		$this->DB()->setQuery($sql);		
		$municipio = $this->DB()->GetSingle();
				
		return $municipio;
	}
	
	function PrimerUltimoDiaSem(){
		...
		$card['primerDia'] = date("Y-m-d",mktime(0,0,0, $mes, $dia-$diaSemana+1, $anio));
		
		# A la fecha recibida, le sumamos el dia de la semana menos siete y obtendremos el domingo
		$card['ultimoDia'] = date("Y-m-d",mktime(0,0,0, $mes, $dia + (7 - $diaSemana), $anio));
		
		return $card;
		
	}//PrimerUltimoDiaSem
	
	function GetDiffDays($fechaIni, $fechaFin){

		$segundos=strtotime($fechaIni) - strtotime($fechaFin);
	...
	function NombreDia($fecha){
		...
		$sql = "SELECT CONCAT(ELT(WEEKDAY('".$fecha."') + 1, 
			'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sabado', 'Domingo')) AS DIA_SEMANA";
			$this->DB()->setQuery($sql);
		$diaSemana = $this->DB()->GetSingle();	
	...
	function OperacionesFecha($fecha, $cantidad = 0, $operacion = "suma", $type = "day"){
		...
		$nuevafecha = strtotime ( $operacion , strtotime ( $fecha ) ) ;
		return  $nuevafecha = date ( 'Y-m-d' , $nuevafecha );
	
	}
	
	function ChangeKey( $result = array(), $newKey){
		//cambia el key del array
		$new_array = array();
		foreach($result as $key => $row){
			$new_array[$row[$newKey]] = $row;
		}
		return $new_array;
	}
