...
function FPDF($orientation='P', $unit='mm', $format='A4')
{
	...
	$this->PageFormats=array('a3'=>array(841.89,1190.55), 'a4'=>array(595.28,841.89), 'a5'=>array(420.94,595.28),
		'letter'=>array(612,792), 'legal'=>array(612,1008), 'test'=>array(595.28,350.89));
	if(is_string($format))
...
//******************************************************************
 function GenerarColumna($columna,$w,$h,$x,$y, $margen = 'LTRB', $bandera = true){  
		$this->SetXY($x,$y);
		if(!is_array($columna)){ $columna = array(); }
        foreach($columna as $fila){
		
            $this->CellFitSpace($w,$h, utf8_decode($fila),'1', 0 , 'C', $bandera);
		}
    }
...
function Row($data,$align = "C", $alternar = false )
{
    ...
    for($i=0;$i<count($data);$i++)
    {
      ...
      // $this->Rect($x,$y,$w,$h,"DF");
		...
		$this->Cell($w,$h," ",1,0,"C", $alternar);
		//$this->CellFitSpace($w,$h," ",1, 0 , 'C',false);
        //Print the text
		$this->SetXY($x,$y);
...
function Row_subdivision($data,$align)
{
	//Calculate the height of the row
    $nb=0;
    for($i=0;$i<count($data);$i++)
...
function Row_SinLine_Importe($data,$align)
{
	//Calculate the height of the row
    $nb=0;
    for($i=0;$i<count($data);$i++)
...
function Row_SinLine($data,$align)
{
	//Calculate the height of the row
    $nb=0;
    for($i=0;$i<count($data);$i++)
...
function Row_SinLineBold($data,$align)
{
	//Calculate the height of the row
    $nb=0;
    for($i=0;$i<count($data);$i++)
...
//=================BARCODE GENERAR CODIGO DE BARRA ===================================//
function EAN13($x,$y,$barcode,$h=5,$w=.35)
{
    //$this->Barcode($x,$y,$barcode,$h,$w,13);
	$this->Barcode($x,$y,$barcode,$h,$w,25);
}

function UPC_A($x,$y,$barcode,$h=5,$w=.35)
{
    //$this->Barcode($x,$y,$barcode,$h,$w,12);
	 $this->Barcode($x,$y,$barcode,$h,$w,23);
}
...
//========================Convertir Cantidades a Texto================================//
		...
		*/
		function centenas($centenas){
				...
				101=>'quinientos',102=>'setecientos',103=>'novecientos',104=>'dos cientos',105=>'tres cientos',106=>'cuatro cientos',107=>'seis cientos',108=>'ocho cientos');*/
				
				$valores = array('cero','uno','dos','tres','cuatro','cinco','seis',
				'siete','ocho','nueve','diez','once','doce','trece','catorce',
				'quince','dieciseis','diecisiete','dieciocho','diecinueve','veinte','veintiún','veintidós','veintitrés','veinticuatro','veinticinco','veintiseis','veintisiete','veintiocho','veintinueve',30=>'treinta',40=>'cuarenta',50=>'cincuenta',
				60=>'sesenta',70=>'setenta',80=>'ochenta',90=>'noventa',100=>'ciento',
				...
				switch($centenas){
						...
						default: return $valores[$VCentena]." - ".$centenas;
					}
			}
		/*
		...
		*/
		function unidades($unidad){
				...
				$valores = array('cero','un','dos','tres','cuatro','cinco','seis','siete','ocho','nueve','diez','once','doce','trece','catorce','quince','dieciseis','diecisiete','dieciocho','diecinueve','veinte','veintiún','veintidos','veintitres','veinticuatro','veinticinco','veintiseis','veintisiete','veintiocho','veintinueve',30=>'treinta',40=>'cuarenta',50=>'cincuenta', 60=>'sesenta',70=>'setenta',80=>'ochenta',90=>'noventa',100=>'ciento',101=>'quinientos',102=>'setecientos',103=>'novecientos',104=>'doscientos',105=>'trescientos',106=>'cuatrocientos',107=>'seiscientos',108=>'ochocientos');
		...
		*/
		function decenas($decena){
				...
				100=>'ciento',101=>'quinientos',102=>'setecientos',103=>'novecientos',104=>'dos cientos',105=>'tres cientos',106=>'cuatro cientos',107=>'seis cientos',108=>'ocho cientos');*/
				
				$valores = array('cero','uno','dos','tres','cuatro','cinco','seis','siete',
				'ocho','nueve','diez','once','doce','trece','catorce','quince','dieciseis','diecisiete','dieciocho','diecinueve','veinte','veintiún','veintidos','veintitres','veinticuatro','veinticinco','veintiseis','veintisiete','veintiocho','veintinueve',30=>'treinta',
				40=>'cuarenta',50=>'cincuenta',60=>'sesenta',70=>'setenta',80=>'ochenta',90=>'noventa',
				100=>'ciento',101=>'quinientos',102=>'setecientos',103=>'novecientos',104=>'doscientos',105=>'trescientos',106=>'cuatrocientos',107=>'seiscientos',108=>'ochocientos');
		...
		*/
		function convertir($valor){
				...
				$tt = $valor;
				$valor = intval($tt);
				//list($Cantidad,$decimales)=split(".",$valor);
				$decimales = $tt - intval($tt);
				
				//$posicionPUnto=strpos(21.129999999888,'.');
				//$decimales=substr(0.129999999888,1,4);
...
//===================================Preferencias Formato con =======================//
//----------------------- comandos disponibles ----------------------
 var $comm=array("[b]","[/b]","[i]","[/i]","[u]","[/u]","[blue]","[red]","[/red]","[black]","[green]","[gray]","[arial]","[courier]","[times]","[helvetica]","[BIG]","[small]","[normal]");
 var $Style=''; 
 ...
 function SetComando($comando)
 {
  if(in_array($comando,$this->comm))
  {
   ...
   if($comando=='[green]') $this->TextColor(0,200,0);
    if($comando=='[gray]') $this->TextColor(128,128,128);
